...
- compact_conversation 摘要不能替代测试证据。
...
- execute 返回 exit_code=124 或 timed_out=true 时，命令已被系统强制终止。不要把超时误报成普通测试失败。
